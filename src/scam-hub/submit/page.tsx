...
type FormState = {
  ...
  media_type: string;
  media_url: string;
  evidence_urls: string;        // raw input (space separated)
  ...
  return (
    <main className="max-w-3xl mx-auto px-4 py-8">
      ...
      <p className="text-gray-500 mb-6">
        Post your experience (eligible for $NOTRUST later) or share a trusted news link.
      </p>
      ...
      {ok ? (
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <label className="block">
            ...
            >
              <option value="experience">Experience (eligible later)</option>
              <option value="curation">Curation (news link)</option>
            </select>
          </label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <input
              className="border rounded p-2"
              placeholder="0x… BSC wallet (optional for now)"
              value={form.submitter_wallet}
              onChange={(e) =>
                setForm((f) => ({ ...f, submitter_wallet: e.target.value }))
              }
            />
            <input
              className="border rounded p-2"
